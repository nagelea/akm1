...
- **定时监控**: 每6小时自动扫描GitHub最新代码
- **自动验证**: 新发现密钥立即验证 + 定时批量验证
- **多平台支持**: 覆盖OpenAI、Anthropic、Google、HuggingFace等20+服务
- **智能过滤**: 自动识别和过滤假密钥、测试密钥
- **可视化面板**: 直观的数据统计和动态趋势图表
...
```
GitHub API ←─── GitHub Actions (扫描 + 验证)
    ↓
数据处理 ←─── Scanner.js (自动验证新密钥)
    ↓
Supabase DB ←─── 数据存储和API (状态追踪)
    ↓
...
3. **启用GitHub Actions**
   - 确保Actions已启用
   - 扫描器每6小时自动运行 (0:00, 6:00, 12:00, 18:00)
   - 验证器每天6点自动运行
