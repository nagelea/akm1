...
RETURNS TABLE (
  total_count BIGINT,
  id INTEGER,
  key_type VARCHAR,     -- 检测到的实际类型
  key_preview VARCHAR,  -- 检测到的实际类型
  severity VARCHAR,     -- 检测到的实际类型
  confidence VARCHAR,   -- 检测到的实际类型
  status VARCHAR,       -- 检测到的实际类型
  repo_name VARCHAR,    -- 检测到的实际类型
  file_path VARCHAR,    -- 检测到的实际类型
  repo_language VARCHAR, -- 检测到的实际类型
  first_seen TIMESTAMPTZ,
  last_verified TEXT,   -- 检测到的实际类型（可能为null）
  context_preview VARCHAR, -- 检测到的实际类型
  full_key VARCHAR,     -- 检测到的实际类型
  raw_context VARCHAR,  -- 检测到的实际类型
  github_url VARCHAR    -- 检测到的实际类型
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
SELECT '✅ All pagination functions created successfully with correct VARCHAR types!' as final_status;